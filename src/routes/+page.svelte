<script lang="ts">
	...
	import CasinoCounter from '$lib/components/CasinoCounter.svelte';
	...
	async function updateCounter() {
		try {
			const counterRef = doc(db, 'stats', 'pageViews');
			...
			if (!snap.exists()) {
				...
				pageViews = newSnap.data()?.count ?? 0; // <-- sicher
			}
		} catch (error) {
	...
	onMount(() => {
		...
		import('firebase/firestore').then(({ onSnapshot }) => {
			const unsubscribe = onSnapshot(counterRef, (docSnap) => {
				pageViews = docSnap.data()?.count ?? 0;
			});
...
<section id="wrapper">
	...
	>
		...
		<p
			id="page-counter"
			class="pointer-events-none absolute bottom-4 left-4 z-10 px-2 py-1 text-sm uppercase"
		>
			[{pageViews}]
		</p>
		...
		<!-- <div class="absolute bottom-0 left-0 z-100 px-4 py-5">
		<ul
			class="max-w-xs md:max-w-md list-none space-y-0.25 text-xs leading-relaxed italic sm:text-xs md:text-xs lg:text-sm"
		>
			<li>
				<span class="not-italic">[RÄTSEL]</span><br />
				Ich bin kein Ding, und doch ist meine Präsenz spürbar. Ich trenne das Sichtbare vom Verborgenen,
				und lege still die Welt in Schweigen. Ich nähre Träume, verberge Sorgen, und erinnere die Sterblichen
				an ihre Endlichkeit. Manchmal fürchtet man mich, manchmal sucht man mich; doch ohne mich gäbe
				es kein Erwachen. Was bin ich?<RiddleInput
					onCorrect={() => (solved = true)}
					onWrong={() => console.log('falsch')}
				/>
			</li>
		</ul>
	</div> -->
